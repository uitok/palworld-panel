...
func Schema() []FieldSchema {
	return localizeSchema([]FieldSchema{
		strField("ServerName", "server_management", "Palworld Server", "服务器名称。会显示在服务端列表和玩家连接信息中。"),
		...
		boolField("RESTAPIEnabled", "server_management", "True", "是否启用官方 REST API。面板监控和部分管理功能会使用它。"),
		...
		boolField("bAllowClientMod", "server_management", "False", "是否允许启用客户端 Mod 的玩家加入服务器。"),
		intField("ChatPostLimitPerMinute", "server_management", "10", 1, 120, "每名玩家每分钟最多可发送的聊天消息数。"),
		strField("CrossplayPlatforms", "server_management", "(Steam,Xbox,PS5,Mac)", "允许连接服务器的平台。默认值为 (Steam,Xbox,PS5,Mac)。"),

		intField("BaseCampMaxNumInGuild", "performance", "4", 1, 10, "每个公会可拥有的最大据点数。提高该值会增加服务器负载。"),
		intField("BaseCampWorkerMaxNum", "performance", "15", 1, 50, "每个据点可工作的最大帕鲁数。提高该值会增加服务器负载。"),
		...
		floatField("PalEggDefaultHatchingTime", "game_balance", "72.0", 0, 240, "巨大蛋孵化所需时间，单位为小时；其他蛋也需要孵化时间。"),
		enumField("DeathPenalty", "game_balance", "All", []string{"None", "Item", "ItemAndEquipment", "All"}, "死亡惩罚。可控制死亡时是否掉落物品、装备和队伍中的帕鲁。"),
		...
		floatField("BlockRespawnTime", "game_balance", "0.0", 0, 3600, "死亡后可再次复活前的冷却时间，单位为秒。"),
		...
		boolField("bIsStartLocationSelectByMap", "features", "True", "是否允许玩家开始游戏时在地图上选择初始位置。"),
		boolField("bHardcore", "features", "False", "是否启用硬核模式。启用后角色死亡将无法复活。"),
		boolField("bShowPlayerList", "features", "True", "是否在 ESC 菜单显示玩家列表。"),
		boolField("bIsUseBackupSaveData", "features", "True", "是否启用世界存档备份。启用后会增加磁盘负载。"),
		boolField("bAllowGlobalPalboxExport", "features", "False", "是否允许保存到全局帕鲁终端。"),
		...
		strField("RandomizerType", "features", "None", "帕鲁刷新随机化模式：None 为不随机，Region 为按区域随机，All 为完全随机。"),
		...
		strField("AdditionalDropItemWhenPlayerKillingInPvPMode", "pvp", "", "启用 PvP 击杀额外掉落后，要掉落的物品 ID。"),
