...
func (s Server) registerWorldRoutes(api *gin.RouterGroup) {
	...
	api.GET("/save-sources/:id/migration/players", Require(PermServerControl), s.saveMigrationPlayers)
	api.POST("/save-migrations/preview", Require(PermServerControl), s.previewSaveMigration)
	api.POST("/save-migrations", Require(PermServerControl), s.startSaveMigration)
