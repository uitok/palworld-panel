...
Mod 页的统一导入流程支持来源输入或本地 ZIP、GitHub ZIP 候选选择、检查结果、更新提示、异步 Job 进度和待重启状态。Workshop 搜索、详情和翻译无需 Steam 登录；Windows 下载前由本机安全管理员配置 Steam 账号和密码，密码保存到 PalPanel 本机受限 secrets 文件，每次下载均显式登录，Steam Guard 验证码只用于当前验证且不保存。Linux/Docker 继续使用服务器终端中的 SteamCMD 登录缓存。GitHub、HTTPS ZIP、本地 ZIP、UE4SS 和 PalDefender 不受 Workshop 登录门禁影响。
