...
describe('PalDefender GM workspace', () => {
  beforeEach(() => {
    ...
    mocks.gmApi.inventory.mockResolvedValue({
      ...
      Inventory: {
        Items: { Available: true, ContainerID: 'container_1', UsedSlots: 1, MaxSlots: 42, FreeSlots: 41, Slots: { 0: { ItemID: 'Money', Count: 25 } } },
  ...
  it('gives items, refreshes inventory, sends a message, and bans the selected player', async () => {
    ...
    await waitFor(() => expect(mocks.gmApi.giveItems).toHaveBeenCalledWith('steam_1', [{ ItemID: 'ExplosiveBullet', Count: 5 }]));
