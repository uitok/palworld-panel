...
export const Monitor: React.FC = () => {
  const { metrics, status } = useServerStore();

  const frameTimeData = useMemo(
    () =>
      Array.from({ length: 8 }).map((_, index) => ({
        time: String(index + 1),
        frame: Number(metrics?.frame_time || 0),
      })),
    [metrics?.frame_time],
  );
  ...
  return (
    <div className="flex flex-col gap-6 p-4 sm:p-6 lg:p-8">
      <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
        ...
        <StatCard title="网络丢包" value="未接入" icon={<Network size={16} />} trend="等待后端指标采集" trendType="info" color="sky" />
      </div>

      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <section className="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_2px_12px_-3px_rgba(15,23,42,0.02)]">
          <h3 className="mb-4 text-[14px] font-bold text-slate-800">网络延迟波动</h3>
          <div className="flex h-72 items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50 text-xs font-semibold text-slate-400">
            当前后端未接入网络延迟采集
          </div>
        </section>
