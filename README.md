...
- 在“系统设置 → 网络与代理”中分别配置服务器安装/更新代理和社区服查询代理；代理密码不会回显，保存后从下一次任务生效
...
如果 SteamCMD 提示 `Steam needs to be online to update`，进入“系统设置 → 网络与代理”，为“服务器安装与更新”填写可用的 `http://`、`https://`、`socks5://` 或 `socks5h://` 地址并启用。Windows SteamCMD 执行时会临时使用当前用户代理，任务结束后自动恢复；面板不会在 API、日志或任务消息中返回代理密码。
